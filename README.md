...
- 添加数据卷挂载支持（config、output、history、logs）
- 配置容器自动重启策略
- 更新部署文档
...
**新闻抓取模块优化**
- 优化 ZAKER 新闻抓取：增加多种选择器策略，放宽过滤条件（标题 > 3 字符即可）
- 优化新浪新闻抓取：使用精确的 `.blk_*` 选择器，放宽过滤条件（标题 > 5 字符）
- 优化网易新闻抓取：增加多种选择器策略
- 新增腾讯新闻源作为第四备用新闻源
- 改进抓取流程：每个新闻源都输出详细日志，方便调试
- 增加去重机制，避免重复抓取同一新闻

**新闻源优先级**
```
ZAKER → 新浪新闻 → 网易新闻 → 腾讯新闻 → 分类新闻
```
...
**原因分析：**
- ZAKER 等网站采用动态加载，普通 HTTP 请求只能获取初始 HTML
- 选择器与网站 HTML 结构不匹配
- 过滤条件过于严格

**解决方案：**
- 项目已优化为多新闻源抓取（ZAKER、新浪、网易、腾讯）
- 放宽了过滤条件，增加多种选择器策略
- 查看日志了解每个新闻源的抓取数量
- 如仍不足，可检查网络连接或尝试其他新闻源
